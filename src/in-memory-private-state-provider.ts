...
/**
 * Browser-friendly PrivateStateProvider (in-memory, scoped by contract address).
 * Adapted from Midnight's Leaderboard / example-bboard UI pattern.
 */
...
> => {
  ...
  return {
    ...
    setSigningKey(addr: ContractAddress, signingKey: SigningKey): Promise<void> {
      signingKeys.set(addr, signingKey);
      return Promise.resolve();
    },
    ...
    importSigningKeys(
      ...
      for (const [addr, key] of Object.entries(payload.keys ?? {})) {
        const address = addr as ContractAddress;
